...
type SandboxSpec struct {
	...
	// KeyPath is the host path to the per-session key file handed to the sandbox.
	// The gVisor (OCI) path delivers the key via a tmpfs bundle file and leaves this
	// empty; process/Docker isolators that pass cmd/sandbox an explicit --key use it.
	KeyPath string
	...
	// — bound into the sandbox at a fixed path so an agent can reach
	...
	// MCPSocket is an OPTIONAL per-session host unix socket — the MCP broker
	// — bound into the sandbox at a fixed path so the agent can use the tools of
	// gateway-approved MCP servers. Empty (the default) binds no MCP socket, leaving
	// the sandbox with no MCP surface. Like the egress socket it is a host-mediated
	// unix socket, not a NIC, so the sandbox stays network=none either way; unlike the
	// shared egress socket it is PER SESSION, so the socket itself is the trusted
	// session identity the broker gates on.
	MCPSocket string

	// Model provider selection. All optional; empty selects the default
	...
	// SearchBackend selects the web_search tool's provider (e.g. "duckduckgo" or
	// "brave[:cred]"), passed to the sandbox as a non-secret flag. It only takes effect
	// alongside EgressSocket — web_search rides the egress broker — so an empty
	// EgressSocket leaves the sandbox sealed regardless of this value. Empty (the
	// default) registers no search tool.
...
type Handle interface {
	...
	// Alive reports whether the sandbox is still running. The Manager calls it on a
	// tracked handle before treating the session as live: a sandbox can die
	// out-of-band (crash, OOM, a manual `docker rm`) and leave a stale handle here,
	// which would otherwise strand the session — inbound messages queue unprocessed —
	// until the sweep's 30-minute heartbeat ceiling reaps it. Alive returns false
	// when the runtime reports the container gone or no longer running, prompting an
	// immediate relaunch. It is best-effort and biased against false negatives: a
	// transient/ambiguous runtime-query error reports true, so a flaky liveness probe
	// never tears down a sandbox that cannot be proven dead.
...
// Alive reports whether the container is still running, via `<runtime> state <id>`
// (the OCI state command, supported by runsc and kata-runtime alike). A non-zero
// exit means the runtime has no such container — `runsc run` reaps the container
// when its init process exits, so a vanished sandbox surfaces here as an error,
// which we treat as not-alive so the Manager relaunches. (A tracked handle implies
// the runtime binary existed at launch, so an exec error is the gone-container
// signal rather than a missing binary.) A parseable state is alive only while the
// container is creating/created/running; "stopped" — exited but not yet deleted —
// is not.
func (h *runscHandle) Alive(ctx context.Context) bool {
	bin := h.runtimeBinary
	if bin == "" {
		bin = DefaultRuntimeBinary
	}
	out, err := exec.CommandContext(ctx, bin, "state", h.containerID).Output()
	if err != nil {
		return false
	}
	var st struct {
	...
	if err := json.Unmarshal(out, &st); err != nil {
		// Unparseable state output: don't tear down on a parsing quirk.
		return true
	}
	switch st.Status {
	case "creating", "created", "running":
		return true
	default: // "stopped" or anything unexpected → relaunch
		return false
	}
}
