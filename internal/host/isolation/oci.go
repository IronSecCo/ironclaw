...
const (
	...
	// broker). It is bound only when SandboxSpec.EgressSocket is set; when
	...
	// containerMCPSock is the OPTIONAL per-session MCP broker unix socket.
	// Bound only when SandboxSpec.MCPSocket is set; the sandbox reaches MCP servers'
	// tools exclusively through it and never speaks MCP directly. network=none either
	// way (it is a host-mediated socket, not a NIC).
	containerMCPSock   = "/run/ironclaw/mcp.sock"
...
func BuildOCISpec(spec SandboxSpec) (*OCISpec, error) {
	...
	// Model provider selection: pass non-secret provider/model/host flags to
	...
	// Egress: when an egress socket is bound (above), tell the sandbox process to use
	// it at the in-container path so http_fetch/web_search actually register. Without
	// this flag the bound socket is unreachable to the tools (the sandbox defaults
	// --egress-socket to empty), so an opted-in session would silently get no egress.
	if spec.EgressSocket != "" {
		args = append(args, "--egress-socket", containerEgressSock)
	}
	// MCP: when a per-session MCP socket is bound (below), tell the sandbox to register
	// the approved MCP tools over it at the in-container path. Without this flag the
	// bound socket is unreachable to the tools (the sandbox defaults --mcp-socket empty).
	if spec.MCPSocket != "" {
		args = append(args, "--mcp-socket", containerMCPSock)
	}
	if spec.SearchBackend != "" {
		args = append(args, "--search-backend", spec.SearchBackend)
	}
	...
	// Optional egress-broker unix socket. Bound only when an EgressSocket is
	...
	// Optional per-session MCP broker unix socket. Bound only when an MCPSocket
	// is configured (the group has gateway-approved MCP grants). rw (a socket needs
	// write to connect) and, being a host-mediated unix socket, it keeps the sandbox
	// network=none.
	if spec.MCPSocket != "" {
		mounts = append(mounts, OCIMount{
			Destination: containerMCPSock,
			Type:        "bind",
			Source:      spec.MCPSocket,
			Options:     []string{"bind", "rw"},
		})
	}
	...
	// (ro,nosuid,nodev,noexec). None set => spec unchanged.
