...
// one remaining external integration point (rootfs provisioning, gap G-008).
//
...
// host-side action (the sandbox is network=none with no package install; see
// AGENTS.md §10). A nil provisioner preserves the original behavior: the caller
...
// and portable; the T-012 spike notes bind/reflink alternatives for hosts that
...
// so it adds no Go dependency to this deliberately stdlib-only tree (see the
// T-012 spike, .agents/spikes/rootfs.md).
//
...
// hosts that permit it (see the T-012 spike).
