...
func ParsePolicyEngine(s string) (PolicyEngine, error) {
	...
	default:
		return "", fmt.Errorf("unknown policy engine %q; use kyverno or gatekeeper", s)
	}
}
...
var policyRules = map[string]policyRule{
	"user.nonroot":     {kyverno: kyvernoRunAsNonRoot, gatekeeper: gkRunAsNonRoot},
	"caps.dropped":     {kyverno: kyvernoDropCaps, gatekeeper: gkDropCaps},
	"seccomp":          {kyverno: kyvernoSeccomp, gatekeeper: gkSeccomp},
	"rootfs.readonly":  {kyverno: kyvernoReadonly, gatekeeper: gkReadonly},
	"network.isolated": {kyverno: kyvernoHostNetwork, gatekeeper: gkHostNetwork},
	"namespaces.host":  {kyverno: kyvernoHostNamespaces, gatekeeper: gkHostNamespaces},
	"docker.sock":      {kyverno: kyvernoDockerSock, gatekeeper: gkDockerSock},
}
...
func EmitPolicy(reports []Report, engine PolicyEngine) (string, error) {
	...
	default:
		return "", fmt.Errorf("unknown policy engine %q; use kyverno or gatekeeper", engine)
	}
}
