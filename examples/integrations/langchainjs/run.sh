#!/usr/bin/env bash
# LangChain.js + IronClaw sandbox -- one-command demo.
#
# Brings up the offline demo control-plane (mock provider -- no model key, no
# channel tokens), then runs a LangChain.js agent whose `sandboxed_shell` tool
...
#
#   examples/integrations/langchainjs/run.sh           # build + up + demo + tear down
#   examples/integrations/langchainjs/run.sh --keep     # leave the demo running
#   examples/integrations/langchainjs/run.sh --attach   # use an already-running demo
#
...
echo "==> running the LangChain.js sandbox demo"
