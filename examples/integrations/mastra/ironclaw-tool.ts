/**
 * Mastra tool backed by an IronClaw sandbox.
 *
 * Mastra agents execute tools created with `createTool`. This is a drop-in
 * replacement for a host-executing shell/code tool: the agent calls it exactly
 * the same way, but every command runs inside an isolated IronClaw per-session
 * sandbox instead of on your machine.
 *
 ...
 *     const agent = new Agent({ tools: { sandboxedShell: makeSandboxTool(sandbox) }, ... });
 */
...
/** Wrap a live IronClaw sandbox as a Mastra tool. */
export function makeSandboxTool(sandbox: IronClawSandbox) {
  return createTool({
    id: "sandboxed_shell",
