#!/usr/bin/env bash
# LangGraph + IronClaw sandbox -- one-command demo.
#
# Brings up the offline demo control-plane (mock provider -- no model key, no
# channel tokens), then drives a LangGraph `ToolNode` whose `sandboxed_shell`
# tool executes inside a real IronClaw per-session sandbox. It runs one benign
# task and a battery of escape attempts and prints a PASS/FAIL containment table.
#
#   examples/integrations/langgraph/run.sh           # build + up + demo + tear down
#   examples/integrations/langgraph/run.sh --keep     # leave the demo running
#   examples/integrations/langgraph/run.sh --attach   # use an already-running demo
#
...
echo "==> running the LangGraph sandbox demo"
