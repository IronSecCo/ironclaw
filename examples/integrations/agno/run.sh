#!/usr/bin/env bash
# Agno + IronClaw sandbox -- one-command demo.
#
# Brings up the offline demo control-plane (mock provider -- no model key, no
# channel tokens), then runs an Agno agent whose `sandboxed_shell` tool executes
...
#
#   examples/integrations/agno/run.sh           # build + up + demo + tear down
#   examples/integrations/agno/run.sh --keep     # leave the demo running
#   examples/integrations/agno/run.sh --attach   # use an already-running demo
#
...
echo "==> running the Agno sandbox demo"
