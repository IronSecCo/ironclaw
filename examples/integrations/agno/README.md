# Agno agents, sandboxed by IronClaw

Your Agno agent runs untrusted, model-generated code. Point that execution at an
...
with IronClawSandbox() as sandbox:                 # engage a per-session sandbox
    toolkit = IronClawTools(sandbox)               # a real Agno Toolkit
    print(toolkit.sandboxed_shell("id"))           # runs INSIDE the sandbox, not your host
    # agent = Agent(tools=[toolkit])               # ... drop into any Agno agent
```
...
tool exactly as Agno's function-calling loop would, runs one benign task plus a
...
Set `OPENAI_API_KEY` (and uncomment `openai` in
[`requirements.txt`](requirements.txt)) and `run.sh` drives a real Agno agent
...
- [`ironclaw_tool.py`](ironclaw_tool.py) — wraps that sandbox as an
  `agno.tools.Toolkit` exposing a `sandboxed_shell` tool. **This is the ~20 lines
  you copy** to swap a host shell tool for a sandboxed one.
- [`run.py`](run.py) — engages the sandbox and drives the tool.

Agno turns the toolkit method's type hints and docstring into the LLM tool
schema; the model calls `sandboxed_shell`, and the command lands inside the box.
