#!/usr/bin/env bash
# Vercel AI SDK + IronClaw sandbox -- one-command demo.
#
# Brings up the offline demo control-plane (mock provider -- no model key, no
# channel tokens), then runs a Vercel AI SDK agent whose `sandboxedShell` tool
...
#
#   examples/integrations/vercel-ai-sdk/run.sh           # build + up + demo + tear down
#   examples/integrations/vercel-ai-sdk/run.sh --keep     # leave the demo running
#   examples/integrations/vercel-ai-sdk/run.sh --attach   # use an already-running demo
#
...
ensure_demo_up "$@"
setup_node "$SCRIPT_DIR"

echo "==> running the Vercel AI SDK sandbox demo"
( cd "$SCRIPT_DIR" && ./node_modules/.bin/tsx run.ts )
