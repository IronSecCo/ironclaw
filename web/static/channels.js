// IronClaw web console — Channels & wiring management (T-223).
//
...
const Channels = (() => {
  ...
  async function loadChannel(id) {
    ...
    if (!id) {
      host.replaceChildren(el("p", { class: "muted", text: "Enter a messaging group id." }));
      return;
    }
  ...
  async function createMessagingGroup() {
    ...
    if (!body.channelType || !body.platformID) {
      setStatus("channel type and platform id are required", "error");
      return;
    }
    try {
      const mg = await api("/v1/registry/messaging-groups", { method: "POST", body: JSON.stringify(body) });
      setStatus("created messaging group " + (mg.ID || ""), "ok");
      $("ch-mg-id").value = mg.ID || "";
  ...
  async function createWiring() {
    ...
    if (!body.messagingGroupID || !body.agentGroupID) {
      setStatus("messaging group id and agent group id are required", "error");
      return;
    }
    try {
      const wr = await api("/v1/registry/wirings", { method: "POST", body: JSON.stringify(body) });
      setStatus("created wiring " + (wr.ID || ""), "ok");
  ...
  async function addDestination() {
    ...
    if (!body.agentGroupID || !body.channelType || !body.platformID) {
      setStatus("agent group, channel, and platform are required", "error");
      return;
    }
    try {
      await api("/v1/registry/destinations", { method: "POST", body: JSON.stringify(body) });
      setStatus("added destination", "ok");
