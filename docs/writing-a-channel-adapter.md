...
- **Standard library only.** Use `net/http` + `encoding/json`. No SDKs, no new
  dependencies (adding one needs `lock:dependency`).
...
pattern used by Teams / Signal / iMessage. Editing `cmd/controlplane/main.go` is a
single-owner shared file — coordinate per [`AGENTS.md`](../AGENTS.md). Then add a row
to [`docs/channels.md`](channels.md) documenting the new credential.
