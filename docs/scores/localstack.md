---
title: "localstack:4 container isolation score: 48/100 (grade D)"
description: "How isolated is localstack:4 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities, runs as root. Scan any container in 10s."
---

# localstack:4 container isolation score: 48/100 (grade D)

Run with plain `docker run localstack/localstack:4` defaults, no hardening flags, the **localstack** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run localstack` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name localstack-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own localstack the same way this page was generated
ironctl scan my-localstack
```
