---
title: "mailhog:v1.0.1 container isolation score: 63/100 (grade C)"
description: "How isolated is mailhog:v1.0.1 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# mailhog:v1.0.1 container isolation score: 63/100 (grade C)

Run with plain `docker run mailhog/mailhog:v1.0.1` defaults, no hardening flags, the **mailhog** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as mailhog (uid != 0) |
...
Applying these to your `docker run mailhog` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name mailhog-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own mailhog the same way this page was generated
ironctl scan my-mailhog
```
