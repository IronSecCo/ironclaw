---
title: "fluentd:v1.17 container isolation score: 63/100 (grade C)"
description: "How isolated is fluentd:v1.17 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# fluentd:v1.17 container isolation score: 63/100 (grade C)

Run with plain `docker run fluent/fluentd:v1.17` defaults, no hardening flags, the **fluentd** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as fluent (uid != 0) |
...
Applying these to your `docker run fluentd` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name fluentd-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own fluentd the same way this page was generated
ironctl scan my-fluentd
```
