---
title: "pgadmin4:8 container isolation score: 63/100 (grade C)"
description: "How isolated is pgadmin4:8 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# pgadmin4:8 container isolation score: 63/100 (grade C)

Run with plain `docker run dpage/pgadmin4:8` defaults, no hardening flags, the **pgadmin4** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run pgadmin4` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name pgadmin4-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own pgadmin4 the same way this page was generated
ironctl scan my-pgadmin4
```
