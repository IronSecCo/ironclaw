...
Run with plain `docker run prom/prometheus:v3.1.0` defaults, no hardening flags, the **prometheus** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run prometheus` targets the biggest gaps first (most points at stake first):
