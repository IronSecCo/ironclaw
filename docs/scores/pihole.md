---
title: "pihole:2024.07.0 container isolation score: 48/100 (grade D)"
description: "How isolated is pihole:2024.07.0 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# pihole:2024.07.0 container isolation score: 48/100 (grade D)

Run with plain `docker run pihole/pihole:2024.07.0` defaults, no hardening flags, the **pihole** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run pihole` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name pihole-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own pihole the same way this page was generated
ironctl scan my-pihole
```
