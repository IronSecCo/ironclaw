---
title: "teamcity-server:2024.12 container isolation score: 63/100 (grade C)"
description: "How isolated is teamcity-server:2024.12 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# teamcity-server:2024.12 container isolation score: 63/100 (grade C)
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as tcuser:tcuser (uid != 0) |
...
Applying these to your `docker run teamcity-server` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name teamcity-server-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  jetbrains/teamcity-server:2024.12
```
...
# grade your own teamcity-server the same way this page was generated
ironctl scan my-teamcity-server
```
...
Maintain **teamcity server** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/teamcity-server/)
