---
title: "duplicati:2.1.0 container isolation score: 48/100 (grade D)"
description: "How isolated is duplicati:2.1.0 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# duplicati:2.1.0 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run duplicati` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name duplicati-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  lscr.io/linuxserver/duplicati:2.1.0
```
...
# grade your own duplicati the same way this page was generated
ironctl scan my-duplicati
```
...
Maintain **duplicati** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/duplicati/)
