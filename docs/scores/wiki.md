---
title: "wiki:2 container isolation score: 63/100 (grade C)"
description: "How isolated is wiki:2 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# wiki:2 container isolation score: 63/100 (grade C)

Run with plain `docker run requarks/wiki:2` defaults, no hardening flags, the **wiki** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run wiki` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name wiki-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own wiki the same way this page was generated
ironctl scan my-wiki
```
