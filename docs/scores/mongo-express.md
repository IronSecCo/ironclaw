---
title: "mongo-express:1.0 container isolation score: 48/100 (grade D)"
description: "How isolated is mongo-express:1.0 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# mongo-express:1.0 container isolation score: 48/100 (grade D)

Run with plain `docker run mongo-express:1.0` defaults, no hardening flags, the **mongo express** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run mongo-express` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name mongo-express-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own mongo-express the same way this page was generated
ironctl scan my-mongo-express
```
