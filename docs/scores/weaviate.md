---
title: "weaviate:1.28.1 container isolation score: 48/100 (grade D)"
description: "How isolated is weaviate:1.28.1 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# weaviate:1.28.1 container isolation score: 48/100 (grade D)

Run with plain `docker run semitechnologies/weaviate:1.28.1` defaults, no hardening flags, the **weaviate** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run weaviate` targets the biggest gaps first (most points at stake first):
...
# grade your own weaviate the same way this page was generated
