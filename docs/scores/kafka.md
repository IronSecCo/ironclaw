---
title: "kafka:3.9.0 container isolation score: 63/100 (grade C)"
description: "How isolated is kafka:3.9.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# kafka:3.9.0 container isolation score: 63/100 (grade C)

Run with plain `docker run apache/kafka:3.9.0` defaults, no hardening flags, the **kafka** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run kafka` targets the biggest gaps first (most points at stake first):
...
# grade your own kafka the same way this page was generated
