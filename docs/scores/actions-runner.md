---
title: "actions-runner:2.321.0 container isolation score: 63/100 (grade C)"
description: "How isolated is actions-runner:2.321.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# actions-runner:2.321.0 container isolation score: 63/100 (grade C)
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as runner (uid != 0) |
...
Applying these to your `docker run actions-runner` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name actions-runner-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  ghcr.io/actions/actions-runner:2.321.0
```
...
# grade your own actions-runner the same way this page was generated
ironctl scan my-actions-runner
```
...
Maintain **actions runner** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/actions-runner/)
