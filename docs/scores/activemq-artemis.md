---
title: "activemq-artemis:2.38.0 container isolation score: 63/100 (grade C)"
description: "How isolated is activemq-artemis:2.38.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# activemq-artemis:2.38.0 container isolation score: 63/100 (grade C)
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as artemis (uid != 0) |
...
Applying these to your `docker run activemq-artemis` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name activemq-artemis-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own activemq-artemis the same way this page was generated
ironctl scan my-activemq-artemis
```
...
Maintain **activemq artemis** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/activemq-artemis/)
