---
title: "unit:1.34.1-minimal container isolation score: 48/100 (grade D)"
description: "How isolated is unit:1.34.1-minimal by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# unit:1.34.1-minimal container isolation score: 48/100 (grade D)

Run with plain `docker run unit:1.34.1-minimal` defaults, no hardening flags, the **unit** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run unit` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name unit-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own unit the same way this page was generated
ironctl scan my-unit
```
