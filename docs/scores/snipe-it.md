---
title: "snipe-it:v7.0.13 container isolation score: 48/100 (grade D)"
description: "How isolated is snipe-it:v7.0.13 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# snipe-it:v7.0.13 container isolation score: 48/100 (grade D)

Run with plain `docker run snipe/snipe-it:v7.0.13` defaults, no hardening flags, the **snipe it** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run snipe-it` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name snipe-it-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  snipe/snipe-it:v7.0.13
```
...
# grade your own snipe-it the same way this page was generated
ironctl scan my-snipe-it
```
