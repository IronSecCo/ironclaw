---
title: "mattermost-team-edition:10.2 container isolation score: 63/100 (grade C)"
description: "How isolated is mattermost-team-edition:10.2 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in..."
---

# mattermost-team-edition:10.2 container isolation score: 63/100 (grade C)
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as mattermost (uid != 0) |
...
Applying these to your `docker run mattermost-team-edition` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name mattermost-team-edition-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  mattermost/mattermost-team-edition:10.2
```
...
# grade your own mattermost-team-edition the same way this page was generated
ironctl scan my-mattermost-team-edition
```
