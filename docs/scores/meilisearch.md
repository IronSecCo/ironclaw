---
title: "meilisearch:v1.11 container isolation score: 48/100 (grade D)"
description: "How isolated is meilisearch:v1.11 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# meilisearch:v1.11 container isolation score: 48/100 (grade D)

Run with plain `docker run getmeili/meilisearch:v1.11` defaults, no hardening flags, the **meilisearch** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run meilisearch` targets the biggest gaps first (most points at stake first):
...
# grade your own meilisearch the same way this page was generated
