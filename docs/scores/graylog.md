---
title: "graylog:6.1 container isolation score: 63/100 (grade C)"
description: "How isolated is graylog:6.1 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# graylog:6.1 container isolation score: 63/100 (grade C)

Run with plain `docker run graylog/graylog:6.1` defaults, no hardening flags, the **graylog** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as graylog (uid != 0) |
...
Applying these to your `docker run graylog` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name graylog-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own graylog the same way this page was generated
ironctl scan my-graylog
```
