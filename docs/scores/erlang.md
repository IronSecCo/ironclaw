---
title: "erlang:27-alpine container isolation score: 48/100 (grade D)"
description: "How isolated is erlang:27-alpine by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# erlang:27-alpine container isolation score: 48/100 (grade D)
...
> Graded from a read-only inspect of a **running container** started from `erlang:27-alpine` at digest `sha256:84699c567f71c7bac12ed97ba50923cfe26b74d9982f984c91c165e06f61ea9e` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run erlang` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name erlang-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own erlang the same way this page was generated
ironctl scan my-erlang
```
