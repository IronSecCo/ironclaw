---
title: "kibana:8.16.1 container isolation score: 63/100 (grade C)"
description: "How isolated is kibana:8.16.1 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# kibana:8.16.1 container isolation score: 63/100 (grade C)

Run with plain `docker run kibana:8.16.1` defaults, no hardening flags, the **kibana** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run kibana` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name kibana-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own kibana the same way this page was generated
ironctl scan my-kibana
```
...
Maintain **kibana** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/kibana/)
