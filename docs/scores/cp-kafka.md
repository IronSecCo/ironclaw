---
title: "cp-kafka:7.8.0 container isolation score: 63/100 (grade C)"
description: "How isolated is cp-kafka:7.8.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# cp-kafka:7.8.0 container isolation score: 63/100 (grade C)

Run with plain `docker run confluentinc/cp-kafka:7.8.0` defaults, no hardening flags, the **cp kafka** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run cp-kafka` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name cp-kafka-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own cp-kafka the same way this page was generated
ironctl scan my-cp-kafka
```
