...
> No base OS image ships fully isolated by default: the leaders still leave capabilities, egress, or a writable root filesystem open. Every one of those gaps closes with a handful of `docker run` flags, listed on every scorecard.
