...
Run with plain `docker run redis:7-alpine` defaults, no hardening flags, the **redis** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run redis` targets the biggest gaps first (most points at stake first):
