---
title: "mailpit:latest container isolation score: 48/100 (grade D)"
description: "How isolated is mailpit:latest by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# mailpit:latest container isolation score: 48/100 (grade D)

Run with plain `docker run axllent/mailpit:latest` defaults, no hardening flags, the **mailpit** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.

> Graded from a read-only inspect of a **running container** started from `axllent/mailpit:latest` at digest `sha256:c96991d9bef73594c246d89ca81411d4e916f03e76a7d2d72fa2ab5dd3c9ce24` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run mailpit` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name mailpit-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own mailpit the same way this page was generated
ironctl scan my-mailpit
```
