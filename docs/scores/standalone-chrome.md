---
title: "standalone-chrome:4.27.0 container isolation score: 63/100 (grade C)"
description: "How isolated is standalone-chrome:4.27.0 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# standalone-chrome:4.27.0 container isolation score: 63/100 (grade C)
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as 1200 (uid != 0) |
...
Applying these to your `docker run standalone-chrome` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name standalone-chrome-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own standalone-chrome the same way this page was generated
ironctl scan my-standalone-chrome
```
...
Maintain **standalone chrome** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/standalone-chrome/)
