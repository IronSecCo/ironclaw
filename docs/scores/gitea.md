...
Run with plain `docker run gitea/gitea:1.22` defaults, no hardening flags, the **gitea** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run gitea` targets the biggest gaps first (most points at stake first):
