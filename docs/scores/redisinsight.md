---
title: "redisinsight:latest container isolation score: 63/100 (grade C)"
description: "How isolated is redisinsight:latest by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# redisinsight:latest container isolation score: 63/100 (grade C)

Run with plain `docker run redis/redisinsight:latest` defaults, no hardening flags, the **redisinsight** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run redisinsight` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name redisinsight-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own redisinsight the same way this page was generated
ironctl scan my-redisinsight
```
