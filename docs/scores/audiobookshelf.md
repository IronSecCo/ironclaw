---
title: "audiobookshelf:2.17.5 container isolation score: 48/100 (grade D)"
description: "How isolated is audiobookshelf:2.17.5 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# audiobookshelf:2.17.5 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run audiobookshelf` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name audiobookshelf-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  ghcr.io/advplyr/audiobookshelf:2.17.5
```
...
# grade your own audiobookshelf the same way this page was generated
ironctl scan my-audiobookshelf
```
...
Maintain **audiobookshelf** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/audiobookshelf/)
