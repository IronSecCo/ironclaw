---
title: "redis-stack-server:7.4.0-v3 container isolation score: 48/100 (grade D)"
description: "How isolated is redis-stack-server:7.4.0-v3 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# redis-stack-server:7.4.0-v3 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run redis-stack-server` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name redis-stack-server-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own redis-stack-server the same way this page was generated
ironctl scan my-redis-stack-server
```
