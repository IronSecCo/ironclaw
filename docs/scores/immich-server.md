---
title: "immich-server:v1.123.0 container isolation score: 48/100 (grade D)"
description: "How isolated is immich-server:v1.123.0 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# immich-server:v1.123.0 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run immich-server` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name immich-server-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own immich-server the same way this page was generated
ironctl scan my-immich-server
```
...
Maintain **immich server** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/immich-server/)
