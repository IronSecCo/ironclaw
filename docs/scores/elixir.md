---
title: "elixir:1.18-alpine container isolation score: 48/100 (grade D)"
description: "How isolated is elixir:1.18-alpine by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# elixir:1.18-alpine container isolation score: 48/100 (grade D)
...
> Graded from a read-only inspect of a **running container** started from `elixir:1.18-alpine` at digest `sha256:e2d0695e6e44eb8d275c4b599e42f660cdbdc617ae318ff81b730e464233b12e` with plain `docker run` defaults, its entrypoint overridden with `sleep` purely to keep it alive. The scan itself executes nothing inside the container. Scoring an image reference instead of a running container yields a different, non-comparable result. [How scoring works &rarr;](../scan.md)
...
Applying these to your `docker run elixir` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name elixir-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own elixir the same way this page was generated
ironctl scan my-elixir
```
