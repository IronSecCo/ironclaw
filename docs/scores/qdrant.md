---
title: "qdrant:v1.12.4 container isolation score: 48/100 (grade D)"
description: "How isolated is qdrant:v1.12.4 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# qdrant:v1.12.4 container isolation score: 48/100 (grade D)

Run with plain `docker run qdrant/qdrant:v1.12.4` defaults, no hardening flags, the **qdrant** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ❌ FAIL | 0/15 | runs as root (user "0:0"); a container escape starts with host-uid 0 |
...
Applying these to your `docker run qdrant` targets the biggest gaps first (most points at stake first):
...
# grade your own qdrant the same way this page was generated
