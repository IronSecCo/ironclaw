---
title: "manticore:6.3.6 container isolation score: 48/100 (grade D)"
description: "How isolated is manticore:6.3.6 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# manticore:6.3.6 container isolation score: 48/100 (grade D)

Run with plain `docker run manticoresearch/manticore:6.3.6` defaults, no hardening flags, the **manticore** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run manticore` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name manticore-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own manticore the same way this page was generated
ironctl scan my-manticore
```
...
Maintain **manticore** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/manticore/)
