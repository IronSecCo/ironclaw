...
Run with plain `docker run prom/node-exporter:v1.8.2` defaults, no hardening flags, the **node exporter** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run node-exporter` targets the biggest gaps first (most points at stake first):
