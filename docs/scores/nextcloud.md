---
title: "nextcloud:30-apache container isolation score: 48/100 (grade D)"
description: "How isolated is nextcloud:30-apache by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# nextcloud:30-apache container isolation score: 48/100 (grade D)

Run with plain `docker run nextcloud:30-apache` defaults, no hardening flags, the **nextcloud** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run nextcloud` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name nextcloud-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own nextcloud the same way this page was generated
ironctl scan my-nextcloud
```
