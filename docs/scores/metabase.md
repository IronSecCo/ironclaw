---
title: "metabase:v0.52.4 container isolation score: 48/100 (grade D)"
description: "How isolated is metabase:v0.52.4 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# metabase:v0.52.4 container isolation score: 48/100 (grade D)

Run with plain `docker run metabase/metabase:v0.52.4` defaults, no hardening flags, the **metabase** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run metabase` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name metabase-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  metabase/metabase:v0.52.4
```
...
# grade your own metabase the same way this page was generated
ironctl scan my-metabase
```
