---
title: "spark:3.5.3 container isolation score: 63/100 (grade C)"
description: "How isolated is spark:3.5.3 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# spark:3.5.3 container isolation score: 63/100 (grade C)

Run with plain `docker run apache/spark:3.5.3` defaults, no hardening flags, the **spark** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as spark (uid != 0) |
...
Applying these to your `docker run spark` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name spark-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own spark the same way this page was generated
ironctl scan my-spark
```
...
Maintain **spark** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/spark/)
