---
title: "minio:latest container isolation score: 48/100 (grade D)"
description: "How isolated is minio:latest by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities, runs as root. Scan any container in 10s."
---

# minio:latest container isolation score: 48/100 (grade D)

Run with plain `docker run minio/minio:latest` defaults, no hardening flags, the **minio** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run minio` targets the biggest gaps first (most points at stake first):
...
# grade your own minio the same way this page was generated
