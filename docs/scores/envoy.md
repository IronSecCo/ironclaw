---
title: "envoy:v1.32-latest container isolation score: 48/100 (grade D)"
description: "How isolated is envoy:v1.32-latest by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# envoy:v1.32-latest container isolation score: 48/100 (grade D)

Run with plain `docker run envoyproxy/envoy:v1.32-latest` defaults, no hardening flags, the **envoy** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run envoy` targets the biggest gaps first (most points at stake first):
...
# grade your own envoy the same way this page was generated
