---
title: "zipkin:3.4.4 container isolation score: 63/100 (grade C)"
description: "How isolated is zipkin:3.4.4 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# zipkin:3.4.4 container isolation score: 63/100 (grade C)

Run with plain `docker run openzipkin/zipkin:3.4.4` defaults, no hardening flags, the **zipkin** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
|-----------|:-------:|------:|---------------------|
| Non-root user (uid != 0) | ✅ PASS | 15/15 | runs as zipkin (uid != 0) |
...
Applying these to your `docker run zipkin` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name zipkin-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
  --security-opt=no-new-privileges \
  --read-only --tmpfs /tmp \
  --network=none \
  openzipkin/zipkin:3.4.4
```
...
# grade your own zipkin the same way this page was generated
ironctl scan my-zipkin
```
...
Maintain **zipkin** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 63/100 C](https://img.shields.io/badge/container%20isolation-63%2F100%20C-d4a72c)](https://ironsecco.github.io/ironclaw/scores/zipkin/)
