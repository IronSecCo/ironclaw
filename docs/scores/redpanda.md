---
title: "redpanda:v24.2.11 container isolation score: 63/100 (grade C)"
description: "How isolated is redpanda:v24.2.11 by default? IronClaw scores its sandbox posture 63/100 (C): retains default capabilities. Scan any container in 10s."
---

# redpanda:v24.2.11 container isolation score: 63/100 (grade C)

Run with plain `docker run redpandadata/redpanda:v24.2.11` defaults, no hardening flags, the **redpanda** image scores **63/100, grade C (partial)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run redpanda` targets the biggest gaps first (most points at stake first):
...
# grade your own redpanda the same way this page was generated
