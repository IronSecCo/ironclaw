---
title: "nginx-proxy-manager:2.12.1 container isolation score: 48/100 (grade D)"
description: "How isolated is nginx-proxy-manager:2.12.1 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# nginx-proxy-manager:2.12.1 container isolation score: 48/100 (grade D)
...
Applying these to your `docker run nginx-proxy-manager` closes the biggest gaps first (most points recovered first):
...
```bash
docker run -d --name nginx-proxy-manager-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own nginx-proxy-manager the same way this page was generated
ironctl scan my-nginx-proxy-manager
```
...
Maintain **nginx proxy manager** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/nginx-proxy-manager/)
