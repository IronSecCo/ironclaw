---
title: "act_runner:0.2.11 container isolation score: 48/100 (grade D)"
description: "How isolated is act_runner:0.2.11 by default? IronClaw scores its sandbox posture 48/100 (D): retains default capabilities. Scan any container in 10s."
---

# act_runner:0.2.11 container isolation score: 48/100 (grade D)

Run with plain `docker run gitea/act_runner:0.2.11` defaults, no hardening flags, the **act_runner** image scores **48/100, grade D (porous)** on IronClaw's seven-dimension container containment scale. Higher is safer. This is what you get straight out of a copy-pasted `docker run`; the fixes below show where the lost points are.
...
Applying these to your `docker run act_runner` targets the biggest gaps first (most points at stake first):
...
```bash
docker run -d --name act_runner-hardened \
  --user 65532:65532 \
  --cap-drop=ALL \
...
# grade your own act_runner the same way this page was generated
ironctl scan my-act_runner
```
...
Maintain **act_runner** (or run it)? Show its default-config isolation score with a badge that links back to this scorecard:

[![Container Isolation Score: 48/100 D](https://img.shields.io/badge/container%20isolation-48%2F100%20D-e8873a)](https://ironsecco.github.io/ironclaw/scores/act_runner/)
